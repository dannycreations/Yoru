...
import { Adapter, BetterSQLite3Database, Database, drizzle, patchDialect, SqliteDatabase } from './';
...
it.effect = (name: string, self: () => Effect.Effect<void, never>, timeout?: number) =>
  itBase.effect(name, () => Effect.provideService(self(), SqliteDatabase, db), timeout);
...
beforeEach(() => {
  ...
  Effect.runSync(Effect.provideService(officeAdapter.insert(sampleOffices), SqliteDatabase, db));
  Effect.runSync(Effect.provideService(userAdapter.insert(sampleUsers), SqliteDatabase, db));
  Effect.runSync(Effect.provideService(postAdapter.insert(samplePosts), SqliteDatabase, db));
});
